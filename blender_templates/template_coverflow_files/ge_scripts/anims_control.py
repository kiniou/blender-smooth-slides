...
grab_push = obj['OBControl']['grab_push']
...
#				faceup_len = fabs(diff) * 0.01 * cm
				...
				faceup_len = ( 1 - fabs(dx) ) * (-1.70 + (1.0*grab_push)) * cm

#			if (dx < 1.0 and dx > -1.0):
			slide.position = [ (i * 0.1* cm ) + (1.0 * cm * dx) , faceup_len ,0 ]
#			else :
...
if sens['control_grab'].triggered and sens['control_grab'].positive :
	set_prop('grab_mode',bool(int(sens['control_grab'].bodies[0]) ) )
...
#	update_real_slide()   
	...
	if ( prop['grab_mode'] == True ):
		if ( sens['control_incr'].positive ) :
			
			incr = obj['OBControl']['grab_val']
