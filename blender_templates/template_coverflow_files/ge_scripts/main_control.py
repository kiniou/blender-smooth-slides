...
prop['grab_push'] = control_obj['grab_push']
...
if prop['grab_mode'] == False:
	...
	if sens['mouse_grab'].positive:
		key_str = '[mouse]'
		set_prop('grab_val',3.0 * float(prop['mouse_ini'] - sens['mouse_grab'].position[0]) / float(prop['win_width']) )
		grab_push += 0.1
	else :
...
if sens['mouse_grab'].positive and sens['mouse_grab'].triggered:
	set_prop('mouse_ini',sens['mouse_grab'].position[0])
